...
def find_device_id_by_name(name: str):
    ...
    for i, d in enumerate(devs):
        if d.get("name") == name and d.get("max_input_channels", 0) > 0:
            return i, d
    raise RuntimeError(f"Input device named '{name}' not found or has no input channels.\n"
...
print(f"Using '{AGGREGATE_DEVICE_NAME}' (id {device_id}) with {total_in} input channels @ {SAMPLE_RATE} Hz")
...
# Open WAV writers
mic_file = sf.SoundFile(MIC_WAV_PATH, mode="w", samplerate=SAMPLE_RATE,
                        channels=len(MIC_CHANNELS), subtype="PCM_16")
sys_file = sf.SoundFile(SYSTEM_WAV_PATH, mode="w", samplerate=SAMPLE_RATE,
                        channels=len(SYSTEM_CHANNELS), subtype="PCM_16")
mix_file = sf.SoundFile(MIXED_WAV_PATH, mode="w", samplerate=SAMPLE_RATE,
                        channels=2, subtype="PCM_16") if WRITE_MIXED_STEREO else None
...
t_mx  = threading.Thread(target=writer_thread, args=(q_mix, mix_file), daemon=True) if WRITE_MIXED_STEREO else None
t_mic.start()
...
try:
    ...
    if WRITE_MIXED_STEREO:
        mix_file.close()
    print(f"Saved: {MIC_WAV_PATH}, {SYSTEM_WAV_PATH}" + (f", {MIXED_WAV_PATH}" if WRITE_MIXED_STEREO else ""))
